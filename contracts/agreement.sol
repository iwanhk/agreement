...
contract Agreement is ERC721A{
    ...
    constructor ()
    ERC721A("Agreement", "AGREE"){
    }

    function create(string calldata _terms, uint256 _due, address[] calldata partyList, address[] calldata assetsList) external{
        nft.push(AgreementNFT(_terms, partyList, assetsList, new address[](0), new bytes32[](0), block.timestamp, _due));
        nft[nft.length-1].party.push(msg.sender);

        _mint(msg.sender, 1);
    }

    function create(string calldata _terms, uint256 _due, address[] calldata partyList, address[] calldata assetsList, AssetArg[] calldata _assets) external{
        nft.push(AgreementNFT(_terms, partyList, assetsList, new address[](0), new bytes32[](0), block.timestamp, _due));
        nft[nft.length-1].party.push(msg.sender);
    ...
    function sign(uint256 tokenId, AssetArg[] calldata _assets) public tokenExist(tokenId){
    ...
    {
        // bytes[] memory parts;
        return string(abi.encodePacked(nft[tokenId].party.length.toString(), "==", nft[tokenId].partyList.length.toString()));
    }

    function agreement(uint256 tokenId) external view tokenExist(tokenId) returns (string memory) {
        uint256 i;
        ...
        bytes memory _buffer=abi.encodePacked(nft[tokenId].terms, "\n[CONTRACT STATUS]: ", status, "\nSinged-Due Date: ", nft[tokenId].signDate.toString(), "-",  nft[tokenId].dueDate.toString(), "\nSigned By:\n");

        uint256 _partyNum= nft[tokenId].party.length;
        for(i=0; i<_partyNum; i++){
            _buffer= abi.encodePacked(_buffer, uint160(nft[tokenId].party[i]).toHexString(), '\n');
        }
        ...
        for(i=0; i<_keys.length; i++){
            ...
            string memory expired= _verifyAsset(_asset, tokenId)?"Verified: ": "Expired: ";
            _buffer= abi.encodePacked(_buffer, '[', 
                        expired,
    ...
    function _addAssets(address _who, uint256 tokenId, AssetArg [] calldata _assets) internal {
    ...
    function _verifyAsset(Asset memory _asset, uint256 tokenId) internal view returns (bool) {
